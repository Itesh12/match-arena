...
export default function Arena() {
  ...
  // Handle Authentication and Room Join/Leave - Consolidated to prevent Hook mismatches
  useEffect(() => {
    if (!initialized || !isHydrated) return;
    
    // Redirect if not logged in
    if (!user) {
      console.log('[ARENA] No user found after hydration, redirecting to login');
      router.push('/login');
      return;
    }
    ...
    console.log('[ARENA] Joining room:', roomId, 'as', user.username);
  ...
  }, [socket, id, user?.id, (user as any)?._id, initialized, isHydrated]); 
  ...
  const handleAcceptRematch = () => {
    if (socket && id) {
      const roomId = (id as string).toUpperCase();
      socket.emit('rematch_response', { roomId, accept: true });
  ...
  const handleRejectRematch = () => {
    if (socket && id) {
      const roomId = (id as string).toUpperCase();
      socket.emit('rematch_response', { roomId, accept: false });
      setHasRespondedToRematch(true);
      setRematchRequest(null);
    }
  };
        ...
        <div className="relative z-10 max-w-2xl w-full glass rounded-[32px] p-10 text-center shadow-[0_0_50px_rgba(0,0,0,0.5)] border-white/5 animate-float">
          ...
          {!isReviewing ? (
            <>
              ...
              <div className="glass rounded-[32px] p-8 mb-4 border-white/5 text-left relative overflow-hidden max-h-[300px] overflow-y-auto custom-scrollbar">
            ...
            <>
              ...
              <div className="space-y-4 mb-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar text-left">
  ...
  return (
    <div className={`relative min-h-[100dvh] ${currentTheme.bg} text-white overflow-hidden flex flex-col lg:flex-row`}>
      ...
      <div className={`z-20 bg-slate-950/60 backdrop-blur-3xl border-r border-white/10 flex flex-col shadow-[10px_0_30px_rgba(0,0,0,0.5)]
        ...
        fixed inset-y-0 left-0 w-[85%] max-w-[280px] transition-transform duration-300
        lg:relative lg:static lg:w-[320px] lg:h-auto lg:min-h-[100dvh]
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-5 space-y-5 relative overflow-hidden group/board">
              ...
              {leaderboard.length > 0 ? leaderboard.map((p, i) => (
                <div key={p.id} className={`flex items-center justify-between relative z-10 group/item ${p.hasLeft ? 'opacity-30' : ''}`}>
                  <div className="flex items-center gap-4">
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-nano border transition-all duration-500 ${i === 0 ? 'bg-yellow-400 text-black border-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.3)] rotate-3' :
        ...
        {gameStatus === 'waiting' ? (
          <div className="flex-1 flex flex-col overflow-y-auto">
            ...
            <div className="w-full flex flex-col items-center justify-start px-4 sm:px-6 pt-4 pb-6 sm:py-8 space-y-4 sm:space-y-10">              {/* Main Math Arena Card - Perfectly Centered */}
              ...
              <div className="w-full max-w-5xl mx-auto px-4 pb-12 space-y-4 sm:space-y-8 animate-in slide-in-from-bottom-5 duration-700 delay-300">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 sm:gap-5">                  {/* Slots for up to 8 players, centered in their grid */}
            ...
            <div className="flex-1 flex flex-col justify-center">
              ...
              {players.find(p => p.id === socket?.id)?.isEliminated ? (
                <div className="glass rounded-[32px] p-20 text-center shadow-2xl border-orange-500/20 relative overflow-hidden group animate-in zoom-in fade-in duration-700">
                  ...
                  <Skull className="w-24 h-24 text-orange-500 mx-auto mb-4 animate-bounce" />
                  <h2 className="text-4xl font-black mb-4 tracking-tighter">{t('arena.eliminated')}</h2>
                  <p className="text-slate-400 font-medium max-w-sm mx-auto mb-4">{t('arena.spectating')}</p>
                ...
                <>
                  <div className={`glass rounded-[32px] p-10 mb-4 text-center shadow-[0_0_80px_rgba(0,0,0,0.5)] border-white/5 relative overflow-hidden group transition-all duration-300 ${timeLeft < 10 ? 'animate-shake animate-pulse-red border-red-500/40' : ''} ${players.find(p => p.id === socket?.id)?.hasShield ? 'border-yellow-400/40 shadow-[0_0_40px_rgba(250,204,21,0.2)]' : ''}`}>
                    ...
                    {selectedMode !== 'standard' && (
                      <div className={`absolute top-4 right-6 px-3 py-1 rounded-full border text-[8px] font-black uppercase tracking-widest flex items-center gap-2 ${selectedMode === 'sudden_death' ? 'bg-orange-500/10 border-orange-500/30 text-orange-400' : 'bg-purple-500/10 border-purple-500/30 text-purple-400'
                        }`}>
                    ...
                    <div className="mt-8 flex items-center justify-center gap-4 animate-in slide-in-from-bottom-4 duration-500">
                      ...
                      {players.find(p => p.id === socket?.id)?.powerUps?.map((pu, idx) => (
                        <button
                          ...
                          className={`group relative p-4 rounded-2xl border transition-all active:scale-[0.9] hover:scale-110 ${pu === 'shield' ? 'bg-yellow-500/10 border-yellow-500/30 hover:bg-yellow-500/20' : 'bg-blue-500/10 border-blue-500/30 hover:bg-blue-500/20'
                            }`}
                        >
      ...
      {/* Overlays & Decorative Elements - Moved to end of JSX to prevent displacement */}
      <InfoModal
        show={showExitModal}
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-blue-600/10 rounded-full blur-[120px] animate-pulse-slow pointer-events-none"></div>
      <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-purple-600/10 rounded-full blur-[120px] animate-pulse-slow pointer-events-none" style={{ animationDelay: '3s' }}></div>
      <div className="absolute top-[30%] left-[40%] w-[20%] h-[20%] bg-indigo-600/5 rounded-full blur-[80px] animate-pulse-slow pointer-events-none" style={{ animationDelay: '1.5s' }}></div>
    </div>
  );
}
